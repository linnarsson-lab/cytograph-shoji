...
class GLMPCA(Algorithm):
	...
	"""
	def __init__(self, n_factors: int, *, family: str = "poi", epsilon: float = 1e-4, penalty: float = 1.0, batch_keys: List[str] = None, covariate_keys: List[str] = None, **kwargs) -> None:
		"""
	...
	def fit(self, ws: shoji.WorkspaceManager, save: bool = False) -> Tuple[np.ndarray, np.ndarray]:
		...
		if covariates is not None and covariates.shape[1] == 0:
			covariates = None
		size_factors = self.TotalUMIs[:].astype("float32")
		size_factors = size_factors / np.median(size_factors)
		data = ws[ws.SelectedFeatures == True][self.requires["Expression"]]
		if covariates is None:
