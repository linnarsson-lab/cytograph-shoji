...
class PlotKaryotype(Algorithm):
    ...
    def fit(self, ws: shoji.WorkspaceManager, save: bool = False) -> None:
        ...
        chromosomes = np.array([trimchr(c) for c in ws.KaryotypeChromosome[:]])
        ...
        for ch, start, length in zip(self.ChromosomeName[:], self.ChromosomeStart[:], self.ChromosomeLength[:]):
            plt.imshow(ploidy[:, chromosomes == trimchr(ch)], vmin=0, vmax=4, aspect="auto", cmap=cmp, interpolation="none", extent=(start, start + length, 0, n_clusters), origin="lower")
