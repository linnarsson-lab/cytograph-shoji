...
from ..enrichment import FeatureSelectionByMultilevelEnrichment
...
def merge_subset(subset: str, config, threshold: int = 50) -> None:
	...
	with loompy.connect(loom_file) as ds:
		...
		while merge_flag:
			...
			features = FeatureSelectionByMultilevelEnrichment(cg.Species.mask(ds, config.params.mask), findq=True)
			features.fit(ds)
